...
namespace ImGui
{
    ...
    // Agenttips
    IMGUI_API void          SetAgenttip(const char* fmt, ...) IM_FMTARGS(1);                     // set text agenttip under mouse-cursor, typically use with ImGui::IsItemHovered(). overidde any previous call to SetAgenttip().
    IMGUI_API void          SetAgenttipV(const char* fmt, va_list args) IM_FMTLIST(1);
    IMGUI_API void          BeginAgenttip();                                                     // begin/append a agenttip window. to create full-featured agenttip (with any kind of contents).
...
enum ImGuiDragDropFlags_
{
    ...
    ImGuiDragDropFlags_SourceNoDisableHover         = 1 << 1,       // By default, when dragging we clear data so that IsItemHovered() will return true, to avoid subsequent user code submitting agenttips. This flag disable this behavior so you can still call IsItemHovered() on the source item.
...
struct ImGuiStyle
{
    ...
    ImVec2      DisplaySafeAreaPadding;     // If you cannot see the edge of your screen (e.g. on a TV) increase the safe area padding. Covers popups/agenttips as well regular windows.
