...
ImGuiStyle::ImGuiStyle()
{
    ...
    DisplaySafeAreaPadding  = ImVec2(4,4);      // If you cannot see the edge of your screen (e.g. on a TV) increase the safe area padding. Covers popups/agenttips as well regular windows.
...
static void AddWindowToRenderListSelectLayer(ImGuiWindow* window)
{
    ...
    else if (window->Flags & ImGuiWindowFlags_Agenttip)
...
// Not exposed publicly as BeginAgenttip() because bool parameters are evil. Let's see if other needs arise first.
void ImGui::BeginAgenttipEx(ImGuiWindowFlags extra_flags, bool override_previous_agenttip)
{
    ImGuiContext& g = *GImGui;
    char window_name[16];
    ImFormatString(window_name, IM_ARRAYSIZE(window_name), "##Agenttip_%02d", g.AgenttipOverrideCount);
    if (override_previous_agenttip)
        if (ImGuiWindow* window = FindWindowByName(window_name))
            if (window->Active)
            {
                // Hide previous agenttips. We can't easily "reset" the content of a window so we create a new one.
                window->HiddenFrames = 1;
                ImFormatString(window_name, IM_ARRAYSIZE(window_name), "##Agenttip_%02d", ++g.AgenttipOverrideCount);
            }
    ImGuiWindowFlags flags = ImGuiWindowFlags_Agenttip|ImGuiWindowFlags_NoTitleBar|ImGuiWindowFlags_NoMove|ImGuiWindowFlags_NoResize|ImGuiWindowFlags_NoSavedSettings|ImGuiWindowFlags_AlwaysAutoResize;
    Begin(window_name, NULL, flags | extra_flags);
}

void ImGui::SetAgenttipV(const char* fmt, va_list args)
{
    BeginAgenttipEx(0, true);
    TextV(fmt, args);
    EndAgenttip();
}
...
static ImVec2 CalcSizeAutoFit(ImGuiWindow* window, const ImVec2& size_contents)
{
    ...
    if ((flags & ImGuiWindowFlags_Agenttip) != 0)
    {
        // Agenttip always resize. We keep the spacing symmetric on both axises for aesthetic purpose.
...
{
    if (flags & (ImGuiWindowFlags_Agenttip | ImGuiWindowFlags_Popup))
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        if (window->HiddenFrames > 0)
            window->HiddenFrames--;
        if ((flags & (ImGuiWindowFlags_Popup | ImGuiWindowFlags_Agenttip)) != 0 && window_just_activated_by_user)
        {
            window->HiddenFrames = 1;
        ...
        if ((flags & ImGuiWindowFlags_Agenttip) != 0 && !window_pos_set_by_api)
        {
            ...
            if (window->AutoPosLastDirection == ImGuiDir_None)
                window->PosFloat = ref_pos + ImVec2(2,2); // If there's not enough room, for agenttip we prefer avoiding the cursor at all cost even if it means that part of the agenttip won't be visible.
        }

        // Clamp position so it stays visible
        if (!(flags & ImGuiWindowFlags_ChildWindow) && !(flags & ImGuiWindowFlags_Agenttip))
        {
        ...
        if (window_just_activated_by_user && !(flags & ImGuiWindowFlags_NoFocusOnAppearing))
            if (!(flags & (ImGuiWindowFlags_ChildWindow | ImGuiWindowFlags_Agenttip)) || (flags & ImGuiWindowFlags_Popup))
                want_focus = true;
...
bool ImGui::Begin(const char* name, bool* p_open, const ImVec2& size_on_first_use, float bg_alpha_override, ImGuiWindowFlags flags)
{
    ...
    // (1) there are multiple variants of WindowBg (popup, agenttip, etc.) and (2) you can't call PushStyleColor before Begin and PopStyleColor just after Begin() because of how CheckStackSizes() behave.
...
void ImGui::ColorAgenttip(const char* text, const float* col, ImGuiColorEditFlags flags)
{
    ...
    ColorButton("##preview", ImVec4(col[0], col[1], col[2], col[3]), (flags & (ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_AlphaPreviewHalf)) | ImGuiColorEditFlags_NoAgenttip, sz);
    SameLine();
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoAgenttip) && hovered)
        ColorAgenttip(desc_id, &col.x, flags & (ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_AlphaPreviewHalf));
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSidePreview))
    {
        ...
        if ((flags & ImGuiColorEditFlags_NoLabel))
            Text("Current");
        ColorButton("##current", col_v4, (flags & (ImGuiColorEditFlags_HDR|ImGuiColorEditFlags_AlphaPreview|ImGuiColorEditFlags_AlphaPreviewHalf|ImGuiColorEditFlags_NoAgenttip)), ImVec2(square_sz * 3, square_sz * 2));
        if (ref_col != NULL)
        {
            Text("Original");
            ImVec4 ref_col_v4(ref_col[0], ref_col[1], ref_col[2], (flags & ImGuiColorEditFlags_NoAlpha) ? 1.0f : ref_col[3]);
            if (ColorButton("##original", ref_col_v4, (flags & (ImGuiColorEditFlags_HDR|ImGuiColorEditFlags_AlphaPreview|ImGuiColorEditFlags_AlphaPreviewHalf|ImGuiColorEditFlags_NoAgenttip)), ImVec2(square_sz * 3, square_sz * 2)))
            {
...
bool ImGui::BeginDragDropSource(ImGuiDragDropFlags flags, int mouse_button)
{
    ...
    if (source_drag_active)
    {
        ...
        if (!(flags & ImGuiDragDropFlags_SourceNoPreviewAgenttip))
        {
            ...
            BeginAgenttipEx(ImGuiWindowFlags_NoInputs);
        }
